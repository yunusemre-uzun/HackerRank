...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    size_t sizeApples = apples.size();
    size_t sizeOranges = oranges.size();
    size_t i;
    unsigned int numberOfApples = 0;
    unsigned int numberOfOranges = 0;
    int dropPoint;
    for(i=0;i<sizeApples;i++){
        dropPoint = a + apples[i];
        if(dropPoint >= s && dropPoint <= t)
            numberOfApples++;
    }
    for (i = 0; i < sizeOranges; i++) {
        dropPoint = b + oranges[i];
        if (dropPoint >= s && dropPoint <= t)
            numberOfOranges++;
    }
    std::cout << numberOfApples << std::endl,
    std::cout << numberOfOranges << std::endl;
    return;
}
